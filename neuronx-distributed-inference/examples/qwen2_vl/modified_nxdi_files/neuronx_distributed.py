...
_NEURON_SUPPORTED_MODELS: Dict[str, Tuple[str, str]] = {
    ...
    "Qwen2VLForConditionalGeneration":
        ("neuronx_distributed_inference.models.qwen2_vl.modelling_qwen2_vl",
...
class NeuronCausalLM(nn.Module):
    ...
    def forward(self,
        ...
        print("IN NEURON CAUSAL LM")
        print(image_grid_thw.shape)
        # print(pixel_values)
        print(input_ids.shape)
        if positions[0][0] != 0:
            attention_mask = torch.ones(1, int(positions[0][0]), dtype=torch.int32)
        else:
