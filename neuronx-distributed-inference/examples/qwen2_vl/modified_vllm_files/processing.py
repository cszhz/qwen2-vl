...
class BaseMultiModalProcessor(ABC, Generic[_I]):
    ...
    def _apply_hf_processor_text_mm(
        ...
        print(f"_get_hf_mm_data")
        print(processor_data, passthrough_data)
        ...
        print("zz:",processed_data.shape)
        #padding for qwen2_vl
        print("Padding inputs for qwen2_vl")
        ...
        if "pixel_values" in processed_data:
            ...
            print(f"processed_data pixel_values before update shape: {pixel_values.shape}")            
            ...
            processed_data.update({"pixel_values": correct_shape_pixel_values})        
            processed_data.update({"image_grid_thw_pad": torch.tensor([[1, 1, PIXEL_SIZE_MAP[seq_len]]])})
            
            print(f"correct_shape_pixel_values shape: {correct_shape_pixel_values.shape}")
            print(f"processed_data pixel_values shape: {processed_data['pixel_values'].shape}")

        # max_image_tokens = PIXEL_SIZE_MAP[seq_len]//4
        # padded_input_ids, padded_attention = pad_inputs(processed_data["input_ids"], processed_data['attention_mask'], 151652, 151653, 151655, max_image_tokens)

        # processed_data.update({"input_ids": padded_input_ids})
        # processed_data.update({"attention_mask": padded_attention})        
        # print(f"updated input_ids shape: {padded_input_ids.shape}")
        # print(f"updated attention_mask shape: {padded_attention.shape}")
        ###end padding
        ...
        if 'pixel_values' in mm_kwargs:
            print(f"mm_kwargs pixel_values: {mm_kwargs['pixel_values'].shape}")
            mm_kwargs.update({'pixel_values': processed_data['pixel_values']})
            print(f"mm_kwargs pixel_values: {mm_kwargs['pixel_values'].shape}")
    ...
    def _apply_hf_processor_main(
        ...
        *,
        ...
        if isinstance(prompt, str):
            ...
            print(f"calling _apply_hf_processor_text_only")
            prompt_ids = self._apply_hf_processor_text_only(prompt)
        else:
            print(f"calling _apply_hf_processor_tokens_only")
        ...
        #print(f"mm_missing_kwargs: {mm_missing_kwargs['pixel_values'].shape}")
    ...
    def _cached_apply_hf_processor(
        ...
        for modality, kw_items in mm_maybe_cached_kw_items.items():
            for idx, kw_item in enumerate(kw_items):
                print(f"kw_item: {kw_item}")
                if kw_item is None:
                    # print(f"modality {modality}, mm_missing_next_idx: {mm_missing_next_idx[modality]}")
                    # print(f"mm_missing_kwargs: {mm_missing_kwargs['pixel_values'].shape}")
                    ...
                    # print(f"inner kw_item: {kw_item}")
                    # print(f"inner kw_item: {kw_item.data['pixel_values'].data.shape}")
        ...
        #print(mm_kwargs)
        #print(f"mm_missing_kwargs: {mm_kwargs['pixel_values'].shape}")
    ...
    def apply(
        ...
        3. Extract information about the placeholder tokens from the
           processed token IDs.
        """
        print(f"RUNNING APPLY: {mm_data.keys()}")
        ...
        #print(f"after _cached_apply_hf_processor: {mm_kwargs['pixel_values'].shape}")
        ...
        #print(f"APPLY: {mm_kwargs['pixel_values'].shape}")
