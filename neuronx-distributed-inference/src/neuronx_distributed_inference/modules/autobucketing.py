...
def generation_model_bk(
    ...
    if attention_mask_is_removed:
        ...
        if padding_side == "right":
            ...
            tensors[1] = torch.ops.aten.slice(
                attention_mask, dim=1, start=buckets[-1] - bucket, end=buckets[-1]
            )
    ...
    if padding_side == "right":
        ...
        for i, tens in enumerate(tensors):
            ...
            if len(tens.shape) == 1:
                ...
                new_tensors.append(
                    torch.ops.aten.slice(tens, dim=1, start=max_idx - bucket, end=max_idx)
                )
